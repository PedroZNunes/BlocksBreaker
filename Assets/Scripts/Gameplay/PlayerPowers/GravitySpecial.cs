...
[Serializable]
public class GravitySpecial : MonoBehaviour
{
    ...
    [SerializeField] GameObject gravityZonePrefab;
        ...
        if (Input.GetMouseButtonDown(0))
        {
            if (isPlayerFree.value && isShootingAllowed.value)
            {
                if (hasCharges()) 
                {
                    ...
                    {
                        Instantiate(gravityZonePrefab, new Vector3(mousePosition.x, mousePosition.y, 1), Quaternion.identity, parent);
